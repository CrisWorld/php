...
if(!empty($_POST["ho_ten"]) && !empty($_POST["email"]) && !empty($_POST["ten_dn"]) && !empty($_POST["mat_khau"]))
{
  ...
  $flag = 1;
  if (!($_COOKIE["user"])) {
    $user = $_POST["ho_ten"].' - '.$_POST["email"] . ' - '.$_POST["ten_dn"].' - '.$_POST["mat_khau"];
...
<form id="form2" name="form2" method="post" action="">
  <?php
 	if($flag==1)
	{
		...
		echo "<a href='doc_session.php'>Click here to read data from session!</a> <br>";
    echo "<a href='doc_cookie.php'>Click here to read data from cookie!</a>";
		echo "</td></tr></table>";
		}
 ?>
</form>
